# Modification of peak_count-MAE_SpaceGroups.py: now peak properties calculated on the whole
# dataset, not just the validation (==> no MAE calculation is possible)
...
def main():
  ...
  name="Peak_count-DFT_WholeDataset"
  ...
  for run in rr:
    ...
    print("Number of materials in TRAINING+VALIDATION dataset=",n_materials)
    line0="Input file f_in=" + f_in0 +"\n"
    f_out2.write(line0)
    line0="Number of materials in TRAINING+VALIDATION dataset="+str(n_materials)+"\n"
    f_out2.write(line0)
    line0="\n"
    f_out2.write(line0)


    llen=len(data[0]['pdos_elast'])
    print("Make sure the spectral parameters are correct for this data!!!!")
    print(" ")
    print("Input file=",f_in0)
    ...
    for delta in dd:
      ...
      io=0
      for ii in data:
           #if (ii["id"] == "POSCAR-JVASP-311.vasp"):  
           #if (ii["id"] == "POSCAR-JVASP-1333.vasp"):  
           jid=ii["jid"]
           name_jid=jid
           ...
           if np_DFT < 21:
              ...
              for ik,iv in peakind_DFT:
                  ...
                  ff=min_freq + ik*step
                  freq_DFT.append(ff)
              freq_min = min(freq_DFT)
              freq_max = max(freq_DFT)
              width = freq_max - freq_min

              #print("DFT: ",jid," N_peaks=",np_DFT," ","position of peaks:",freq_DFT)
              #print(peakind_DFT)
              #print("")
              #print("min=",freq_min,"  max=",freq_max,"  range=",width)

              if cryst_sym1 in cryst_sym_Npeaks_DFT.keys():
                 ...
                 peak_min_freq[cryst_sym1] = peak_min_freq[cryst_sym1] + freq_min
                 peak_max_freq[cryst_sym1] = peak_max_freq[cryst_sym1] + freq_max
                 range_freq[cryst_sym1]    = range_freq[cryst_sym1] + width
                 peak_min_freq2[cryst_sym1] = peak_min_freq2[cryst_sym1] + freq_min * freq_min
                 peak_max_freq2[cryst_sym1] = peak_max_freq2[cryst_sym1] + freq_max * freq_max
                 range_freq2[cryst_sym1]    = range_freq2[cryst_sym1] + width * width
              else:
                 cryst_sym_count[cryst_sym1] = 1
                 cryst_sym_Npeaks_DFT[cryst_sym1]  = np_DFT
                 cryst_sym_Npeaks_DFT2[cryst_sym1] = np_DFT * np_DFT
                 peak_min_freq[cryst_sym1]  = freq_min
                 peak_min_freq2[cryst_sym1] = freq_min * freq_min
                 peak_max_freq[cryst_sym1]  = freq_max
                 peak_max_freq2[cryst_sym1] = freq_max * freq_max
                 range_freq[cryst_sym1]  = width
                 range_freq2[cryst_sym1] = width * width
              io = io+1
              #if (io > 2): break
              #break
      ...
      line0="\n"
      f_out2.write(line0)
      print("Run=",run,"tollerance=",delta)
      line0="Run="+str(run)+" tollerance="+str(delta)+"\n"
      f_out2.write(line0)
      line0="\n"
      f_out2.write(line0)
      ...
      line0="\n\nDFT Results on complete (Training+Validation) Dataset\n"
      f_out2.write(line0)
      line0="======================================================\n\n"
      f_out2.write(line0)
      ...
      line0="----------------------------------\n"
      f_out2.write(line0)
      line0="Id crystal_system  #_peaks    standard_dev   standard_error   counts\n"
      f_out2.write(line0)
      io=1
      for ik in cryst_sym_Npeaks_DFT.keys():
          ...
          line1=str(io)+" "+str(ik)+"    "+str(np.around(mean_Npeak,3))+" "+str(np.around(st_dev,2))+" "
          line0=line1+" "+str(np.around(st_error,2))+"   "+str(np.around(cryst_sym_count[ik]))+"\n"
          f_out2.write(line0)
          io=io+1
      line0="counts = number of materials in the specific crystalline system\n"
      f_out2.write(line0)

      line0="\n"
      f_out2.write(line0)
      line0="Peak positions/frequency range per crystal system\n"
      f_out2.write(line0)
      line0="-------------------------------------------------\n"
      f_out2.write(line0)
      line0="Id crystal_system     min_freq  st_dev  st_err    max_freq  st_dev  st_err    freq_range  st_dev  st_err\n"
      f_out2.write(line0)
      io=1
      for ik in cryst_sym_Npeaks_DFT.keys():
          mean_min_freqpeak = peak_min_freq[ik]/cryst_sym_count[ik]
          mean_max_freqpeak = peak_max_freq[ik]/cryst_sym_count[ik]
          mean_range_freq = range_freq[ik]/cryst_sym_count[ik]
          mean_min_freqpeak2 = peak_min_freq2[ik]/cryst_sym_count[ik]
          mean_max_freqpeak2 = peak_max_freq2[ik]/cryst_sym_count[ik]
          mean_range_freq2 = range_freq2[ik]/cryst_sym_count[ik]

          var_min = mean_min_freqpeak2 - mean_min_freqpeak * mean_min_freqpeak
          var_max = mean_max_freqpeak2 - mean_max_freqpeak * mean_max_freqpeak
          var_range = mean_range_freq2 - mean_range_freq * mean_range_freq
          st_dev_min   = np.sqrt(var_min)
          st_dev_max   = np.sqrt(var_max)
          st_dev_range = np.sqrt(var_range)
          st_error_min   = st_dev_min/np.sqrt(cryst_sym_count[ik])
          st_error_max   = st_dev_max/np.sqrt(cryst_sym_count[ik])
          st_error_range = st_dev_range/np.sqrt(cryst_sym_count[ik])

          line1=str(io)+" "+str(ik)+" "+str(np.around(mean_min_freqpeak,3))
          line2=" "+str(np.around(st_dev_min,2))+" "+str(np.around(st_error_min,2))+"   "
          line3=str(np.around(mean_max_freqpeak,3))+" "+str(np.around(st_dev_max,2))+" "
          line4=str(np.around(st_error_max,2))+"   "+str(np.around(mean_range_freq,3))
          line5=" "+str(np.around(st_dev_range,2))+" "+str(np.around(st_error_range,2))
          line0=line1+line2+line3+line4+line5+"\n"
          f_out2.write(line0)
          io=io+1
      line0="where: min_freq(max_freq) = lowest (highest) freq. for peaks, range=max_freq-min_freq\n"
      f_out2.write(line0)
